...
var lines = {};
  ...
  for(i=0; i<variables.length; i++){
      lines["intheUK"].push({date:d3.timeParse("%Y")(variables[i].split("y")[1]),amt:+countrydata_allI[0][variables[i]]})
      lines["intheEU"].push({date:d3.timeParse("%Y")(variables[i].split("y")[1]),amt:+countrydata_allE[0][variables[i]]})
  }
...
function highlightcountry(countrycode) {
    ...
    //barcode labels
    d3.select("#importlabel").html("<p>into " + countryname +"</p><p class='labelbold'>£"+ d3.format(",")(importval) +"m</p>")
    d3.select("#exportlabel").html("<p>from " + countryname +"</p><p class='labelbold'>£"+ d3.format(",")(exportval) +"m</p>")

    //Update labels
    d3.select("#tradewith").html("Investments into/from " + countryname + " <span style='font-weight:300'>2016</span>")
...
function unhighlightcountry(countrycode) {
  ...
  //Update labels
  d3.select("#tradewith").html("Investments <span style='font-weight:300'>2016</span>")
...
function createBarcode(lastyear){
  ...
  //Get data
	barcodeimports = dataimports;
	barcodeexports = dataexports;
	...
	maxValue = d3.max([d3.max(barcodeimports, function(d) { return +d[lastyear];}), d3.max(barcodeexports, function(d) { return +d[lastyear];})]);
    ...
    barcodeLabels = d3.select("#barcode")
		...
		.html("<span style='font-weight:bold'>Investments</span> 2016")
		.append("div")
...
function barchartstart(){
    ...
    yAxisLine = d3.axisLeft(yLine)
                  .tickFormat(function(d){return d3.format(".0s")(d)})
                  .ticks(7)
    ...
    //gridlines
		var y_axis_grid = function() { return yAxisLine; }

    line_gen = d3.line()
    .x(function(d) { return xLine(d.date); })
    .y(function(d) { return yLine(+d.amt); });

//make the data in the right format for lines
    linesimports = {}
    for(i=0;i<dataimports.length;i++){
      linesimports[dataimports[i].CountryId]=[]
        for(j=0;j<variables.length;j++){
              linesimports[dataimports[i].CountryId].push({
                'date':d3.timeParse("%Y")(+variables[j].split("y")[1]),
                'amt':+dataimports[i][variables[j]]
              })
          }
        }
    ...
    for(i=0;i<dataexports.length;i++){
      linesexports[dataexports[i].CountryId]=[]
        for(j=0;j<variables.length;j++){
              linesexports[dataexports[i].CountryId].push({
                'date':d3.timeParse("%Y")(+variables[j].split("y")[1]),
                'amt':+dataexports[i][variables[j]]
              })
          }
        }
    ...
    // y domain
    importsmax=d3.max(d3.entries(linesimports), function(c) {
    ...
    exportsmax=d3.max(d3.entries(linesexports), function(c) {
      return d3.max(c.value, function(v) {
        var n = v.amt;
        return Math.ceil(n);
      });
    ...
    importsmin=d3.min(d3.entries(linesimports), function(c) {
    ...
    exportsmin=d3.min(d3.entries(linesexports), function(c) {
									return d3.min(c.value, function(v) {
										var n = v.amt;
										return Math.floor(n);
									});
    ...
    yMax=Math.max(importsmax,exportsmax)
    yMin=Math.min(importsmin,exportsmin)
      ...
      svg.append('g')
       ...
       svg.append('g')
        ...
        svg.append('g').selectAll('path')
 			        .data([[{date:d3.timeParse("%Y")(1999),amt:0},{date:d3.timeParse("%Y")(2016),amt:4000000}]])
        ...
        svg.append('g').selectAll('path')
               .data([[{date:d3.timeParse("%Y")(1999),amt:0},{date:d3.timeParse("%Y")(2016),amt:2000000}]])
               .enter()
               .append('path')
