...
var lines = {};
  ...
  yAxisLine = d3.axisLeft(yLine)
                .tickFormat(function(d){return fmt(d).replace('M','').replace('T','').replace('G','')})
                .ticks(7)
...
function unhighlightcountry(countrycode) {
  ...
  //Update labels
  d3.select("#tradewith").html("Investments <span style='font-weight:300'>2017</span>")
...
function createBarcode(lastyear){
  ...
  //Get data
	barcodeimports = dataimports.filter(function(d){return d.CountryId!="W1" });
	barcodeexports = dataexports.filter(function(d){return d.CountryId!="W1" });
  ...
  minValue = d3.min([d3.min(barcodeimports, function(d) { return +d[lastyear];}), d3.min(barcodeexports, function(d) { return +d[lastyear];})])/1000;
    ...
    barcodeLabels = d3.select("#barcode")
		...
		.html("<span style='font-weight:bold'>Investments</span> 2017")
		.append("div")
	...
	barcodeArea.append("g")
		...
		mobile = true;
		 ...
		 barcodeLabels = d3.select("#barcode")
			...
			.html("<span style='font-weight:bold'>UK Trade</span> 2017")
			.append("div")
